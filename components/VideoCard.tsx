...
const VideoCard = ({
                       id, title, thumbnail, createdAt, userImg, username, views, visibility, duration
                   }: VideoCardProps) => {
    return (
        <Link href={`/video/${id}`} className={"video-card"}>
            ...
            <article>
                <div>
                    <figure>
                        <Image src={userImg || '/asset/images/dummy.jpg'} alt={"avatar"} width={34} height={34} className={"rounded-full aspect-square"} />
                        <figcaption>
                            <h3>{username}</h3>
                            <p>{visibility}</p>
                        </figcaption>
            ...
            {duration && (
                <div className={"duration"}>{Math.ceil(duration/60)} Minutes</div>
            )}
        </Link>
    )
}
